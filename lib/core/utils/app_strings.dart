class AppStrings {
  // App Bar
  static const String appBarProducts = 'المنتجات';

  // Empty State
  static const String emptyProductsTitle = 'No Products Yet';
  static const String emptyProductsSubtitle =
      'Start by adding your first product';
  static const String addProductButton = 'Add Product';

  // Display Toggle
  static const String toggleToVertical = 'تغيير عرض المنتجات الى عمودي';
  static const String toggleToHorizontal = 'تغيير عرض المنتجات الى افقي';

  // Product Details
  static const String productPlaceholderText = 'هذا النص هو مثال لنص';
  static const String storeName = 'اسم المتجر';
  static const String currencyDollar = 'دولار';

  // Categories
  static const String categoriesTitle = 'التصنيفات';
  static const String viewAllCategories = 'عرض الكل';
  static const String category1 = 'تصنيف 1';
  static const String category2 = 'تصنيف 2';
  static const String category3 = 'تصنيف 3';

  // Error Messages
  static const String errorTitle = 'Oops! Something went wrong';
  static const String errorFallback = 'Something went wrong';
  static const String tryAgainButton = 'Try Again';

  // Product Sample Data
  static const String sampleProductName = 'هذا النص هو مثال لنص';
  static const String sampleStoreName = 'اسم المتجر';
  static const String sampleCategory = 'Architecture';
  static const String sampleImageUrl =
      'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?w=400';

  // Category Image URLs
  static const String categoryImage1 =
      'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=200';
  static const String categoryImage2 =
      'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=200';
  static const String categoryImage3 =
      'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?w=200';

  // Comments
  static const String commentAddProductFunctionality =
      'Add product functionality will be handled by the app bar button';
  static const String commentReloadList = 'Reload the list';
  static const String commentKeepExistingData =
      'Keep existing data when reloading';
}
